...
> **核心口号**：*"while tool_use：思考 → 执行 → 反馈 → 再思考 —— 这就是 Agent"*
...
- [ ] **Python 基础**：能读懂 `while`/`for` 循环、列表操作、`dict` 字典、函数定义与调用
- [ ] **API 调用概念**：理解 HTTP 请求-响应模型，知道什么是 JSON 格式
- [ ] **命令行基础**：能在终端中执行 `python`、`pip install`、`cd`、`ls` 等基本命令
- [ ] **环境准备**：已完成 [00-基础篇](./00-basics.md) 中的环境配置，能成功运行 `uv run python agents/s01_agent_loop.py`

> 如果某项不熟悉，不必担心——本章代码总量不到 50 行，边学边查完全可行。环境配置是唯一硬性前提。
...
**阅读方法**：从左上角 "messages[]" 开始，沿箭头顺时针追踪。每个标注都是实际传递的数据结构。注意两个关键分支：`end_turn` 退出循环，`tool_use` 继续循环并将结果追加回 `messages[]`。
...
- `f"You are a coding agent at {os.getcwd()}"` -- 告诉 LLM 当前工作目录。**为什么**：LLM 没有文件系统感知能力，如果不注入路径，它生成的命令中路径可能毫无意义（如引用 `/home/user/` 等猜测路径）。
- `"Use bash to solve tasks"` -- 引导 LLM 使用工具而非空谈。**为什么**：LLM 默认倾向于"解释"而非"执行"，这句提示将行为模式从顾问切换为执行者。
- `"Act, don't explain"` -- 减少不必要的解释，直接执行。**为什么**：在 Agent 场景中，用户关心的是结果而非过程。省略冗余解释可以减少 token 消耗，并让 Agent 感觉更高效。
...
下表汇总了 s01 源码中的所有关键设计决策及其理由，便于快速回顾和对比：

| 决策 | 选择 | 理由 |
|------|------|------|
| 循环条件 | `while True` + `stop_reason` 判断 | 让 LLM 自主决定何时停止，而非预设固定轮次；LLM 天然知道任务是否完成 |
| 消息累积 | 每轮 `append` 而非替换 | LLM 是无状态的，必须每次发送完整历史才能保持上下文连贯 |
| 工具结果角色 | 以 `"user"` 角色追加 | Anthropic API 只有 `user`/`assistant` 两种角色，工具结果不是 assistant 说的，必须用 `user` 承载 |
| 安全防护 | 黑名单子串匹配 + 超时 | 最小可用方案：不依赖外部沙盒，零配置即可运行，代价是防护强度有限 |
| 输出截断 | 50000 字符上限 | 防止单个工具结果耗尽上下文窗口；超出时 LLM 可用 `head`/`tail` 分段读取 |
| 子进程方案 | `subprocess.run` 而非 `os.popen` | 官方推荐、支持 stderr 捕获、原生超时控制；`os.popen` 已标记过时 |
| 系统提示注入 `cwd` | `f-string` 动态拼接 | LLM 无文件系统感知，需要显式告知路径才能生成有意义的命令 |
| 打印截断 | 终端仅显示前 200 字符 | 避免刷屏干扰交互体验；完整输出仍存入消息列表供 LLM 使用 |
...
**`tool_use_id` 的作用**：让 LLM 知道这个结果对应哪个工具调用。**为什么需要 ID 匹配**：LLM 可能在一次响应中并行发起多个工具调用（比如同时读两个文件），每个调用分配唯一的 `id`。如果不匹配 ID，LLM 将无法区分哪个结果对应哪个请求，后续推理会出错。
...
**答案**：
1. 防止单个工具结果消耗过多上下文窗口
2. LLM 的上下文窗口有限（8,000 - 200,000 token），输出过大会导致后续轮次无法携带完整历史
...
**目标**：在每次循环迭代时打印调试信息，观察消息列表的增长过程。

**具体要求**：修改 `agent_loop`，在每次循环开始时打印当前轮次编号和消息数量。
...
**验收标准**：
- 运行 Agent 并输入一个需要 2-3 轮工具调用的任务（如"列出当前目录的 Python 文件"）
- 终端输出中能看到 `--- Round 1, messages: 1 ---`、`--- Round 2, messages: 3 ---` 等递增信息
- 能解释为什么第 2 轮的 messages 数量是 3（提示：1 条用户输入 + 1 条 assistant 响应 + 1 条工具结果）

### 练习 2：添加循环上限（基础）

**目标**：防止 Agent 因 LLM 反复调用工具而陷入无限循环。

**具体要求**：定义 `MAX_ROUNDS = 50` 常量，将 `while True` 改为 `for _ in range(MAX_ROUNDS)`，达到上限时打印警告。

**验收标准**：
- 设置 `MAX_ROUNDS = 3` 后运行 Agent，输入一个复杂任务（如"列出所有文件并逐个分析"）
- Agent 在第 3 轮后停止并打印警告信息
- 正常任务（1-2 轮即可完成）不受影响

### 练习 3：扩展危险命令黑名单（进阶）

**目标**：增强 `run_bash` 的安全防护能力。

**具体要求**：在 `run_bash` 的 `dangerous` 列表中至少添加 3 个新的危险命令模式：`mkfs`（格式化文件系统）、`dd if=`（底层磁盘写入）、`chmod 777`（全权限开放）。

**验收标准**：
- 输入 "格式化磁盘" 后，Agent 尝试的命令被拦截，返回 `Error: Dangerous command blocked`
- 输入 "列出文件" 等正常命令不受影响
- 能解释黑名单方案的局限性（提示：子串匹配可以被 `s\udo` 等方式绕过）

### 练习 4：多工具并行验证（进阶）

**目标**：验证源码已正确处理 LLM 在一次响应中发起多个工具调用的情况。

**具体要求**：修改系统提示，鼓励 LLM 并行调用（如 `You may run multiple independent commands at once`）。然后输入一个能触发并行调用的任务（如"同时查看 file1.py 和 file2.py 的内容"）。

**验收标准**：
- 终端中能看到一次循环迭代打印了两条黄色命令
- `results` 列表中包含两个 `tool_result` 条目，且 `tool_use_id` 各不相同
- LLM 在下一轮能正确区分两个文件的内容并分别回应
