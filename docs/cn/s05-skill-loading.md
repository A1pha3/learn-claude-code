...
> **核心口号**：*"目录常驻，正文按需；知识不在上下文里，在触手可及处"*
...
> **概念速查：两层注入（Two-Layer Injection）**
>
> 一种 Token 高效的知识加载模式。Layer 1（元数据）将技能名称和简短描述写入系统提示，每次调用都发送，但每技能仅约 20 tokens。Layer 2（完整内容）仅在 LLM 主动调用 `load_skill` 工具时才注入，通过 `tool_result` 返回完整技能正文。将 Token 成本从 O(N * T_body) 降至 O(N * T_meta + k * T_body)。

> **概念速查：SkillLoader**
>
> 技能管理核心类。初始化时通过 `rglob("SKILL.md")` 递归扫描 `skills/` 目录，解析每个 SKILL.md 的 YAML frontmatter（name、description、tags）和 Markdown 正文，存入 `self.skills` 字典。对外提供 `get_descriptions()`（Layer 1 输出）和 `get_content()`（Layer 2 输出）两个方法。

> **概念速查：SKILL.md**
>
> 技能定义文件，位于 `skills/<skill-name>/SKILL.md`。格式为 YAML frontmatter（`---` 包裹的 name + description + 可选 tags）+ Markdown 正文（详细规范、步骤、代码示例）。description 应包含 "Use when..." 触发条件，帮助 LLM 判断何时需要加载。
...
**为什么静态注入的浪费特别严重？** 因为 LLM 的注意力是有限的。当系统提示中包含大量与当前任务无关的知识时，模型需要"过滤"这些噪声，这会降低对真正相关信息的关注度。Token 浪费不仅仅是成本问题，更是质量问题。
...
1. **不使用 yaml 库**：没有引入 `import yaml`，而是用正则 + 字符串分割实现轻量解析。**为什么不引入 yaml 库？** 这是一个刻意的技术选择：减少外部依赖使得部署更简单（无需 `pip install pyyaml`），对于 SKILL.md 这种简单的 key: value 格式，正则解析已经足够。代价是牺牲了对多行值的支持，但这个局限可以通过编写规范（要求 description 写成单行）来规避。
...
| 决策 | 选择 | 备选方案 | 理由 |
|------|------|----------|------|
| 注入层数 | 两层（元数据 + 正文） | 单层全量 / 三层（+ 辅助资源） | 两层是成本与灵活性的最优平衡点；三层可作为扩展方向 |
| 技能存储 | 文件系统 `skills/<name>/SKILL.md` | SQLite / JSON 数据库 | Markdown 文件便于编辑和 Git 版本控制，非程序员也能维护 |
| Frontmatter 解析 | 正则 + `split(":", 1)` 轻量解析 | `yaml.safe_load()` 标准库 | 零外部依赖，对简单 key: value 格式足够；代价是不支持多行值 |
| 技能发现 | `rglob("SKILL.md")` 递归扫描 | 配置文件显式注册 | 自动发现减少维护成本，新增技能只需创建目录和文件 |
| Layer 2 包裹格式 | `<skill name="...">...</skill>` XML 标签 | Markdown 代码块 / JSON | XML 标签语义明确，LLM 能准确识别内容边界 |
| 技能 ID 来源 | frontmatter 的 `name` 字段，缺失时用目录名 | 强制要求 `name` 字段 | 容错设计，降低 SKILL.md 的编写门槛 |
...
| 方面 | s04 子 Agent | s05 技能系统 | 为什么这样演进 |
|------|-------------|--------------|----------------|
| 加载时机 | 调用时创建子对话 | 调用时加载静态知识 | s04 解决了"做事时的上下文污染"，s05 解决了"知道时的 Token 浪费"——两者解决不同维度的问题 |
| 内容类型 | 动态推理结果 | 静态参考文档 | 动态内容需要隔离的推理空间，静态内容只需注入到当前上下文 |
| Token 成本 | 中等（子 Agent 的完整循环） | 低（一次性注入） | 技能内容是固定的 Markdown 文本，不需要额外的 LLM 推理循环 |
| 适用场景 | 需要多步推理的子任务 | 需要规范/模板的参考知识 | 推理需要独立上下文，参考知识只需注入即可 |
| 可复用性 | 每次重新推理 | 加载相同内容，可缓存 | 技能内容不变，多次调用可返回同一份文本 |
| 隔离性 | 完全隔离的上下文 | 注入到主上下文中 | 子 Agent 的中间步骤对主 Agent 无价值，但技能知识需要主 Agent 直接引用 |
| 实现复杂度 | 需要独立的 Agent 循环 | 只需一个工具处理器 | 技能加载的本质是文件读取 + 格式化，远比运行一个 Agent 循环简单 |
