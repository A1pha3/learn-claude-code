...
> **概念速查：子 Agent（Sub-agent）**
>
> 由父 Agent 通过 `task` 工具派发的独立 Agent 实例。拥有全新的 `sub_messages` 列表（不继承父历史），使用独立的系统提示 `SUBAGENT_SYSTEM`，工具集限制为 4 个基础工具（不含 task，防止递归）。执行完毕后整个上下文被丢弃，仅将最后一次响应中的文本摘要返回给父 Agent。

> **概念速查：上下文隔离（Context Isolation）**
>
> 父子 Agent 各自维护独立的消息列表。子 Agent 在隔离环境中执行任务，其工具调用、文件读取、中间推理等全部细节不会污染父 Agent 的主上下文。核心价值：主上下文只接收摘要（约 500 tokens），而非全部过程（可能 15,000+ tokens）。

> **概念速查：工具隔离（Tool Isolation）**
>
> `CHILD_TOOLS` 不包含 `task` 工具，从结构上阻止子 Agent 创建孙 Agent（防止无限递归）。`PARENT_TOOLS = CHILD_TOOLS + [task]` 的列表拼接方式是这一约束的实现手段。
...
**为什么不直接在主上下文中执行？** 表面原因是 Token 节省，但更深层的原因是"注意力保护"。当主上下文中堆积了大量文件内容时，LLM 的注意力会被分散到这些细节上，导致系统提示和用户原始需求被"挤出"注意力焦点。上下文隔离不仅节省了 Token，更重要的是保护了 Agent 的决策质量。
...
**为什么只返回最后一次的文本？** 因为子 Agent 的系统提示明确指示 "Complete the given task, then summarize your findings"，所以最后一次响应自然包含了总结。返回所有轮次的文本会违背隔离的初衷——那等于把子上下文的内容整体搬回主上下文。
...
| 决策 | 选择 | 备选方案 | 理由 |
|------|------|----------|------|
| 上下文隔离方式 | 新建 `sub_messages = [{"role": "user", "content": prompt}]` | 复制父上下文后裁剪 | 新建是最彻底的隔离，零泄漏风险；复制裁剪可能遗漏敏感内容 |
| 结果返回形式 | 仅返回最后一次响应的文本摘要 | 返回完整 sub_messages 或结构化 JSON | 主上下文的 Token 预算极其宝贵，摘要是最小化占用的方式 |
| 递归防护 | `CHILD_TOOLS` 不含 task（结构性阻止） | 运行时深度计数器 | 结构性防护比运行时检查更可靠，不可能被绕过 |
| 轮次上限 | 硬编码 `range(30)` | 常量 / 动态计算 | 硬编码简单直接，30 轮足以覆盖绝大多数子任务；常量提取是更好的工程实践，但本质相同 |
| task 工具路由方式 | `agent_loop` 中的条件分支 `if block.name == "task"` | 加入 `TOOL_HANDLERS` 字典 | task 的处理逻辑（创建独立 Agent 循环）远比普通工具复杂，不适合 lambda 路由；且不加入字典可安全共享 `TOOL_HANDLERS` |
| 工具处理器共享 | 父子共用同一个 `TOOL_HANDLERS` 字典 | 子 Agent 使用独立的处理器副本 | 文件操作和 bash 执行的逻辑完全相同，共享减少重复代码 |
...
| 方面 | s03 | s04 | 为什么这样演进 |
|------|-----|-----|----------------|
| 规划方式 | TodoManager（内存状态机） | 子 Agent（独立上下文） | s03 解决了"迷失方向"的问题，但没有解决"上下文污染"——所有工具结果仍在主上下文中；s04 通过隔离彻底解决 |
| 上下文管理 | 单一共享上下文 | 父子隔离 | 文件读取的输出动辄数千 tokens，共享上下文中这些细节会快速消耗 Token 预算 |
| 工具数量 | 5（含 todo） | 4（子）+ 1（父: task）= 5 | 工具总数相同但职责不同：s03 的 todo 管理进度，s04 的 task 委派子任务 |
| 结果返回 | 渲染文本留在主上下文 | 摘要文本返回主上下文 | s03 的渲染文本是完整状态，s04 的摘要是精简结论——后者对 Token 的占用更少 |
| Token 效率 | 中等（所有细节在主上下文） | 高（细节在子上下文中被丢弃） | 子 Agent 模式的 Token 节省率可达 95% 以上 |
| 递归风险 | 无 | 需防护（通过工具隔离解决） | 新增的 task 工具引入了递归可能，必须从结构层面阻止 |
| 提醒机制 | 有（rounds_since_todo >= 3） | 无 | s04 的子 Agent 有 `range(30)` 轮次上限作为安全兜底，不需要额外的偏离提醒 |
