...
> **概念速查：微压缩（Micro-compact）**
>
> 每轮无条件执行的零成本压缩。将超过 KEEP_RECENT（默认 3）个的旧工具结果替换为 `[Previous: used <tool_name>]` 占位符。通过匹配 `tool_use_id` 找到对应的工具名，使占位符保留语义信息。只有原始内容长度超过 100 字符时才触发替换。

> **概念速查：自动压缩（Auto-compact）**
>
> 当 `estimate_tokens(messages)` 超过 THRESHOLD（默认 50000）时触发。先将完整对话历史保存到 `.transcripts/` 目录的 JSONL 文件，再请求 LLM 生成包含三方面（已完成工作、当前状态、关键决策）的摘要，最后用 2 条消息（摘要 + 确认）替换整个历史。

> **概念速查：转录存档（Transcript Archive）**
>
> 压缩前的完整对话历史以 JSONL（JSON Lines）格式保存到 `.transcripts/transcript_<timestamp>.jsonl`。每行一个 JSON 对象，保证可逐行解析和追加。压缩不等于永久丢失——转录文件支持事后审查、调试和信息恢复。
...
**为什么是三层而不是一层？** 因为不同类型的信息有不同的"保质期"和"压缩成本"。微压缩（Layer 1）每轮执行但只处理工具结果，成本几乎为零；自动压缩（Layer 2）处理整个历史但需要一次额外的 LLM 调用，成本较高；手动压缩（Layer 3）让 Agent 在阶段切换时主动清理，是最精准但需要 Agent 有足够的自我意识。三层各有分工，逐层升级。
...
| 决策 | 选择 | 备选方案 | 理由 |
|------|------|----------|------|
| 压缩层数 | 三层（微 / 自动 / 手动） | 单层（仅自动压缩） | 不同粒度的压缩解决不同问题：微压缩零成本每轮执行，自动压缩兜底，手动压缩给 Agent 主动权 |
| 微压缩触发 | 每轮无条件执行 | 仅在 Token 超过阈值时 | 微压缩成本几乎为零（纯字符串替换），每轮执行可保持上下文长期精简 |
| KEEP_RECENT = 3 | 保留最近 3 个工具结果 | 5 / 10 / 动态调整 | 3 个平衡了压缩效果与上下文保留；最近的工具结果通常与当前任务最相关 |
| Token 估算 | `len(str(messages)) // 4` | tiktoken / Anthropic Token API | 零依赖、跨模型兼容、速度快；偏高 10-30% 反而更安全（提前触发压缩） |
| 摘要生成 | 一次额外 LLM 调用 | 本地摘要模型 / 规则截断 | LLM 生成的摘要质量远高于规则截断，一次性成本换来后续所有调用的上下文缩减 |
| 转录格式 | JSONL（每行一条消息） | JSON 数组 / SQLite | JSONL 支持逐行解析、追加友好、容错性好，适合持续写入场景 |
| 转录命名 | Unix 时间戳 `transcript_{timestamp}.jsonl` | UUID / 自增序号 | 时间戳天然排序，人类可读，便于定位特定时间点的对话记录 |
| 压缩后消息结构 | 2 条消息（user 摘要 + assistant 确认） | 仅保留 1 条 user 消息 | 需要一问一答的消息对结构，否则 API 会因消息角色不交替而报错 |
...
**为什么用 `messages[:] =` 而非 `messages = auto_compact(messages)`？** Python 中 `messages = new_list` 只会重新绑定局部变量，不影响外部传入的原始列表。而 `messages[:] = new_list` 通过切片赋值修改原始列表的内容，使得所有持有该列表引用的地方都能看到更新。这是一个常见的 Python 可变对象陷阱。
...
| 方面 | s05 技能系统 | s06 上下文压缩 | 为什么这样演进 |
|------|-------------|----------------|----------------|
| 问题 | 静态知识浪费 Token | 动态对话消耗 Token | s05 解决了"输入侧"的 Token 浪费（不用的知识不该加载），s06 解决了"积累侧"的 Token 膨胀（旧对话不该永久占据空间） |
| 解决方案 | 两层注入（按需加载） | 三层压缩（按需遗忘） | 加载和遗忘是 Token 管理的两面，缺一不可 |
| 触发时机 | LLM 请求时（load_skill） | 每轮（微）+ 阈值（自动）+ 主动（手动） | 知识加载是事件驱动的（LLM 判断需要时才加载），上下文压缩是混合驱动的（定时 + 阈值 + 主动） |
| 存储位置 | skills/ 目录 | .transcripts/ 目录 | 技能是可复用的静态资源，转录是一次性的历史快照 |
| 文件格式 | SKILL.md（Markdown） | transcript_*.jsonl（JSON Lines） | 技能需要人可编辑，转录需要机器可解析且支持追加 |
| 目标 | 减少 Token 成本 | 实现无限会话 | s05 的目标是一次性节省，s06 的目标是长期可持续 |
| 互补性 | 控制知识注入量 | 控制上下文膨胀量 | s05 + s06 共同构成完整的 Token 生命周期管理 |
