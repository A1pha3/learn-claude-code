...
> **"自回归"的含义**：模型每次只生成一个 Token，然后把生成的 Token 加入输入，再预测下一个 Token，如此反复。这就像写作时"一个字一个字地想"——每次只看前面写过的内容，决定下一个字写什么。
...
完整过程：
输入 "法国的首都是" → 预测 "巴" → 输入变为 "法国的首都是巴" → 预测 "黎" → 完成
```

三个关键特征（理解它们是理解 Agent 的前提）：

| 特征 | 含义 | 对 Agent 设计的影响 |
|------|------|---------------------|
| **无状态** | 每次调用都是独立的，LLM 不"记住"之前的对话 | 必须在每次调用时发送完整消息历史 |
| **文本到文本** | 只能接收和生成文本，不能直接操作文件系统或网络 | 必须通过工具调用桥接外部世界 |
| **概率模型** | 输出基于概率分布，同一输入可能产生不同输出 | Agent 的行为具有非确定性，需要错误处理 |
...
**为什么这个局限很重要**：理解了这个局限，就理解了为什么需要 Agent——Agent 让 LLM 拥有了"手"。更具体地说：
- **工具调用**解决了"有手"的问题——LLM 可以指挥外部代码执行操作
- **消息循环**解决了"有脑"的问题——LLM 可以根据执行结果调整下一步操作
- 两者结合，LLM 从"被动回答问题的顾问"变成了"主动完成任务的助手"
...
三个要素缺一不可——缺少任何一个都无法构成 Agent：

| 要素 | 职责 | 缺少会怎样 |
|------|------|-----------|
| **LLM** | 理解意图、制定计划、决策下一步操作 | 没有智能决策能力，退化为普通脚本 |
| **工具** | 将 LLM 的决策转化为实际操作 | 只有想法没有行动，仍然是"被困的顾问" |
| **循环** | 自动处理"调用 → 获取结果 → 继续思考"的闭环 | 每步都需要人工介入，退化为聊天机器人 |

> **关键区分**：LLM + 工具（无循环）= 聊天机器人（单轮问答）；LLM + 循环（无工具）= 自言自语（无法行动）；只有三者结合才是 Agent。
