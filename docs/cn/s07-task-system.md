...
| 层级 | 目标 | 检验标准 |
|------|------|----------|
| ⭐ 基础 | 理解任务持久化与 DAG 概念 | 能解释为什么需要 `.tasks/` 目录而非内存列表，能看懂 `blockedBy`/`blocks` 字段 |
| ⭐⭐ 进阶 | 独立使用 TaskManager 管理中等复杂度项目 | 能设计 10+ 任务的依赖图，正确使用 `create`/`update`/`list_all` 完成完整工作流 |
| ⭐⭐⭐ 专家 | 扩展任务系统并分析性能瓶颈 | 能实现循环依赖检测、`list_all` 排序修复、依赖索引优化，能评估 O(N) 全局扫描的适用边界 |
...
> | 概念 | 定义 | 源码映射 |
> |------|------|----------|
> | **DAG** | Directed Acyclic Graph，有向无环图。节点 = 任务，边 = 依赖关系 | `blockedBy` / `blocks` 字段 |
> | **blockedBy** | "我被谁阻塞"——该任务等待哪些前置任务完成 | `task["blockedBy"]: List[int]` |
> | **blocks** | "我阻塞了谁"——该任务完成后会解除哪些下游任务 | `task["blocks"]: List[int]` |
> | **_clear_dependency** | 状态传播核心：完成任务后全局扫描，从所有下游的 `blockedBy` 中移除自身 | `TaskManager._clear_dependency()` |
> | **状态传播** | 任务完成后自动解除下游阻塞，无需 Agent 手动干预 | `update()` 中 `status == "completed"` 分支 |
...
│  ★ task_list    ★ task_get                       │
│          （s07 新增，操作 TaskManager）            │
│                                                   │
│  ┌──────────────────────────────────┐             │
│  │  ★ TaskManager（s07 核心新组件）  │             │
│  │                                   │             │
│  │  .tasks/                          │             │
│  │  ├── task_1.json                  │             │
│  │  ├── task_2.json                  │             │
...
| 特性 | TodoManager (s03) | TaskManager (s07) | 演进动机 |
|------|-------------------|-------------------|----------|
| 存储 | 内存中的 Python 列表 | 磁盘上的 JSON 文件目录 | 上下文压缩后内存数据丢失，必须持久化到磁盘才能跨会话存活 |
| 持久化 | 无，上下文压缩后丢失 | 有，`.tasks/` 目录跨会话存活 | s06 压缩机制会丢弃非结构化数据，磁盘文件不受压缩影响 |
| 依赖关系 | 无 | `blockedBy` + `blocks` 双向引用 | 扁平列表无法表达任务执行顺序，导致 Agent 可能违反逻辑先后关系 |
| 合法状态 | `pending` / `in_progress` / `completed` | `pending` / `in_progress` / `completed` | 三态模型在教学场景足够，生产系统可扩展 |
| 状态传播 | 无（手动管理） | 自动（`_clear_dependency` 全局扫描） | 手动管理依赖解除容易遗漏，自动化传播消除人为错误 |
| 所有权 | 无 | `owner` 字段（为多 Agent 预留） | s09 多 Agent 团队需要任务认领机制 |
| ID 生成 | 列表索引（连续） | 文件名解析（不连续但唯一） | 文件系统命名天然支持唯一性，且允许删除后 ID 不重用 |
...
| 决策 | 选择 | 原因 | 替代方案 |
|------|------|------|----------|
| 存储粒度 | 每个任务一个 JSON 文件 | 原子写入、并发友好、故障隔离 | 单一 `tasks.json`：简单但并发需加锁，单点损坏风险 |
| ID 生成策略 | `max(existing_ids) + 1` 从文件名解析 | 不需额外计数器文件，重启后自动恢复 | UUID：无碰撞但不可读；自增计数器文件：额外维护负担 |
| 依赖解除机制 | 全局扫描 `_clear_dependency()` | 实现简单，天然幂等，小规模下性能可接受 | 反向索引 `blocks_index` 字典：O(1) 查找但增加维护复杂度 |
| 双向引用维护 | `blockedBy` + `blocks` 同时更新 | 查询只看 `blockedBy`，`blocks` 用于 `add_blocks` 时反向写入 | 单向引用：查询和传播都需遍历，无法 O(1) 判断可执行性 |
| 排序方式 | `sorted(glob("task_*.json"))` 按文件名字典序 | 实现零成本，N < 10 时正确 | 自然排序（`sorted(key=lambda...)`）：N > 9 时才需要，留给练习 |
| 状态模型 | 三态（pending/in_progress/completed） | 教学场景最小完备，避免过度设计 | 五态（+blocked/cancelled）：更精确但增加状态机复杂度 |
...
### 场景一：微服务拆分项目（30+ 任务、复杂依赖）

假设要将一个单体应用拆分为 3 个微服务。先梳理出完整 DAG：

```
task_1: 设计服务边界文档
  └→ task_2: 定义 API 契约（依赖 1）
  └→ task_3: 搭建基础设施（Docker Compose / K8s 配置，依赖 1）
      ├→ task_4: 实现用户服务（依赖 2, 3）
      ├→ task_5: 实现订单服务（依赖 2, 3）
      └→ task_6: 实现支付服务（依赖 2, 3）
          ├→ task_7: 服务间集成测试（依赖 4, 5, 6）
          └→ task_8: 性能压测（依赖 7）
              └→ task_9: 生产部署（依赖 8）
```

Agent 可在第一天设计并创建全部任务文件，第二天直接从 `.tasks/` 恢复状态继续推进。`_clear_dependency` 确保每完成一个上游任务，下游自动解锁。

### 场景二：Sprint 迭代规划

将 Sprint Backlog 中的每个 User Story 映射为一个任务，用 `description` 字段记录验收标准：

```python
task_create(
    subject="用户登录功能",
    description="验收标准：1. 支持 email+密码 2. 错误提示友好 3. 单元测试覆盖率 > 80%"
)
```

每天会话开始时调用 `task_list()` 查看当前进度，不需要外部工具即可掌握全局状态。

### 场景三：多环境部署流水线

```
task_1: 构建 Docker 镜像
  └→ task_2: 部署到 dev 环境（依赖 1）
      └→ task_3: 运行冒烟测试（依赖 2）
          └→ task_4: 部署到 staging（依赖 3）
              └→ task_5: 运行集成测试（依赖 4）
                  └→ task_6: 部署到 production（依赖 5）
```

每一步都是原子操作，任何环节失败可精确回溯到对应的 `task_N.json` 查看状态。
...
| 序号 | 检查项 | 说明 |
|------|--------|------|
| 1 | 是否存在循环依赖？ | 沿 `blockedBy` 链走一遍，确认不会回到起点 |
| 2 | 依赖是否最小化？ | 每条 `blockedBy` 是否真正需要前置任务的产出 |
| 3 | 并行机会是否被充分利用？ | 没有依赖关系的任务应该可以同时推进 |
| 4 | 任务粒度是否适中？ | 单个任务 30 分钟 ~ 2 小时为宜，太细增加管理开销 |
| 5 | 是否有明确的完成标准？ | 每个 `description` 是否写清了"什么叫完成" |
...
**验收标准**：
- `task_create(subject="紧急修复", priority="high")` 返回的 JSON 包含 `"priority": "high"` 字段
- `task_list()` 输出中出现优先级标记，如 `[!] #1: 紧急修复`（high=[!], medium=[~], low=[-]）
- 未指定 priority 时默认为 `"medium"`
- `task_update(task_id=1, priority="high")` 可以修改已有任务的优先级
...
**验收标准**：
- 创建 task_1 和 task_2，设置 task_2 blockedBy [1]
- 调用 `task_update(task_id=1, addBlockedBy=[2])` 应抛出异常，提示存在循环依赖
- 正常的线性依赖链（1->2->3->4）不受影响
- 菱形依赖（1->2, 1->3, 2->4, 3->4）不受影响
- 异常信息中包含循环路径的具体描述
...
**验收标准**：
- 创建 12 个任务后，`task_list()` 输出顺序为 1, 2, 3, ..., 10, 11, 12（而非 1, 10, 11, 12, 2, 3, ...）
- 修复方法使用 `key` 参数提取文件名中的数字进行自然排序
- 修复后 ID 为 1 位的任务（1-9）和 2 位的任务（10-12）混合排列时仍然正确
...
**验收标准**：
- 创建菱形依赖（1->2, 1->3, 2->4, 3->4）后，`task_dot()` 输出合法的 DOT 语法
- 输出可直接通过 `dot -Tpng` 渲染为图片
- 节点标签包含任务 ID 和 subject（如 `1: 项目初始化`）
- 已完成的任务和未开始的任务使用不同节点样式（颜色或形状）
